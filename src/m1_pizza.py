...
def run_test_generate_points_on_circle():
    ...
    # Test 2:
    expected = [rg.Point(150, 200.0),  # All numbers are approximate.
                rg.Point(100, 250),
                rg.Point(50, 200),
                rg.Point(100, 150)]
...
def run_test_draw_points_on_circle():
    ...
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4:  100 cyan dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'red'
    draw_points_on_circle(window, circle, 100, 'cyan')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  175 red slices on black circle.'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'black'
    pizza(window, circle, 175, 'red', 3)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # Tests 1 and 2 (on the same window):
    title = ('POLYGON test 3:  8 segments with thicc orange lines')
    window = rg.RoseWindow(550, 400, title)

    circle = rg.Circle(rg.Point(100, 100), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 4, 'orange', 20)
    window.continue_on_mouse_click()
...
def run_test_fancy_polygon():
    ...
    title = ('FANCY POLYGON test 3:  300 lime green lines on cyan circle,'
             + ' hops = 61.')
    window = rg.RoseWindow(1000, 1000, title)

    circle = rg.Circle(rg.Point(500, 350), 150)
    circle.fill_color = 'blue'
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
            lines = rg.Line(points[k - hops_to_next_point], points[k])
            lines.thickness = thickness
            lines.color = color
            lines.attach_to(window)
